...
export default function UploadMarketPricePage() {
  ...
  const handleExtractSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (
        ...
        setUploadStatus({
          message: `Successfully extracted ${result.length} records. Review data below and confirm import.`,
          error: undefined,
        });
      ...
      setUploadStatus({
        message: "",
        error: `Extraction Error: ${errorMessage}`,
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCommit = async () => {
    ...
    try {
      const response = await fetch("/admin/market-price/commit", {
        // Commit endpoint
        method: "POST",
        headers: {
          ...
          <form onSubmit={handleExtractSubmit}>
            ...
            <div className="mb-4 p-6 border border-dashed border-gray-300 dark:border-gray-600 rounded-md text-center">
              ...
              <input
                ...
                className="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 dark:file:bg-red-900 dark:file:text-red-300 hover:file:bg-red-100 dark:hover:file:bg-red-800 cursor-pointer"
              />
